...
import { CustomerProfilePage } from "@pages/profile/ProfilePages";
...
function requireTempUser(tempUser: CustomerProfileState | null): CustomerProfileState {
  expect(tempUser, "Temp customer profile user must be created by the scenario fixture").toBeTruthy();
  return tempUser!;
}

base.describe("Customer - Profile @regression", () => {
  ...
  const test = base.extend<{ scenarioSetup: void }>({
    scenarioSetup: [
      async ({ page, testState, pageObjects: _pageObjects, navigationPage }, use) => {
        ...
        await navigationPage.open("/customer/profile");
  ...
  test("[E2E-CUS-PRO-001] should current account information display when profile overview", async ({
    page: _page,
    pageObjects,
    ...
    await steps.act("perform profile overview behavior", async () => {
      ...
      const profilePage = pageObjects.create(CustomerProfilePage);
  ...
  test("[E2E-CUS-PRO-002] should success sweetalert display when success message", async ({
    page: _page,
    pageObjects,
    navigationPage,
    steps
    ...
    await steps.act("perform success message behavior", async () => {
      const profilePage = pageObjects.create(CustomerProfilePage);
      await navigationPage.open("/customer/profile?successMessage=Cap%20nhat%20thanh%20cong");
  ...
  test("[E2E-CUS-PRO-003] should rejection without password confirmation when username update", async ({
    page: _page,
    pageObjects,
    navigationPage: _navigationPage,
    steps
    ...
    await steps.act("perform username update behavior", async () => {
      ...
      await profilePage.submitUsernameChange(TestDataFactory.uniqueUsername("cust"), "000000");
      await profilePage.waitForSweetAlertContains(/loi|that bai|error|mat khau|otp/i);
      await profilePage.confirmSweetAlertIfPresent();

      expect(await readProfileUsername("customer", activeUser.id)).toBe(activeUser.username);
      expect((await profilePage.readProfileValues()).username).toBe(originalValues.username);
    });

    await steps.assert("verify rejection without password confirmation", async () => {
      expect.soft(true, "verification checkpoint: verify rejection without password confirmation").toBe(true);
    });
  });

  test("[E2E-CUS-PRO-004] should successful update with valid OTP when phone number update", async ({
    page: _page,
    testState,
    pageObjects,
    navigationPage: _navigationPage,
    ...
    await steps.act("perform phone number update behavior", async () => {
      ...
      await profilePage.waitForSweetAlertContains(/thanh cong|cap nhat so/i);
      await expect.poll(() => readProfilePhone("customer", activeUser.id)).toBe(newPhone);
    });
  ...
  test("[E2E-CUS-PRO-005] should client-side mismatch validation when password confirmation", async ({
    page: _page,
    pageObjects,
    navigationPage: _navigationPage,
    steps
  ...
  test("[E2E-CUS-PRO-006] should successful update with valid OTP and re-login when password update", async ({
    page,
    testState,
    pageObjects,
    navigationPage: _navigationPage,
    ...
    await steps.act("perform password update behavior", async () => {
      ...
      const oldHash = await readProfilePasswordHash("customer", activeUser.id);
      ...
      await expect.poll(() => readProfilePasswordHash("customer", activeUser.id)).not.toBe(oldHash);
